...
void OnSurfaceCreatedCB(OH_NativeXComponent *component, void *window) {
    ...
    OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "Callback", "OnSurfaceCreatedCB: id %{public}s", id.c_str());
    if ((xSize == OH_NATIVEXCOMPONENT_RESULT_SUCCESS) && (render != nullptr) && (id == "srcPlayer")) {
        if (render->eglCore_->EglContextInit(window, width, height)) {
            ...
            OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "Callback", "create windowOut : w:%lu h:%lu",width,height);
        }
    }
    if ((xSize == OH_NATIVEXCOMPONENT_RESULT_SUCCESS) && (render != nullptr) && (id == "dstPlayer")) {
        if (render->eglCore_->EglContextInit(window, width, height)) {
            auto context = PluginManager::GetInstance();
