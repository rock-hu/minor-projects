...
function getAllDiagnostics(
  ...
  if (isStrict) {
    return !builderProgram.builderProgramForLinter ?
      [] :
      builderProgram.builderProgramForLinter.getSemanticDiagnostics(sourceFile, cancellationToken).filter((diag) => {
        return diag.file === sourceFile;
      });
  }
  return builderProgram.getSemanticDiagnostics(sourceFile, cancellationToken).filter((diag) => {
    return diag.file === sourceFile;
  });
}
