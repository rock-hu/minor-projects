...
export interface TestModeProperties {
  resultFileExt: string;
  mode: TestMode;
  modeOpts: LinterOptions;
}

export const DEFAULT_MODE_PROPERTIES: TestModeProperties = {
  resultFileExt: '.json',
...
const MIGRATE_MODE_PROPERTIES: TestModeProperties = {
  ...
  modeOpts: {
    migratorMode: true,
    noMigrationBackupFile: true
  }
};

export const testModePropsMap: Map<string, TestModeProperties> = new Map([
  ['default', DEFAULT_MODE_PROPERTIES],
  ['autofix', AUTOFIX_MODE_PROPERTIES],
  ['arkts2', ARKTS2_MODE_PROPERTIES],
  ['migrate', MIGRATE_MODE_PROPERTIES]
]);
