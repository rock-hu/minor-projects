...
void e2p_test_plugin_dump_json_Initialize()
{
    puts("Hi there!");
    puts("---");
...
void e2p_test_plugin_dump_json_AfterParse(es2panda_Context *ctx)
{
    puts("After parse:");
    puts("---");
    es2panda_AstNode *ast = impl->ProgramAst(ctx, impl->ContextProgram(ctx));
    impl->AstNodeForEach(ast, DumpJSON, ctx);
}

void e2p_test_plugin_dump_json_AfterCheck(es2panda_Context *ctx)
{
    puts("After check:");
    puts("---");
}

void e2p_test_plugin_dump_json_AfterLowerings(es2panda_Context *ctx)
{
