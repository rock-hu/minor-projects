...
{
    if (g_impl->IsAssignmentExpression(ast)) {
        auto left = g_impl->AssignmentExpressionLeft(g_ctx, ast);
        g_impl->AssignmentExpressionSetResult(g_ctx, ast, left);
        return g_impl->AssignmentExpressionResult(g_ctx, ast) == left;
    }
    return false;
}
