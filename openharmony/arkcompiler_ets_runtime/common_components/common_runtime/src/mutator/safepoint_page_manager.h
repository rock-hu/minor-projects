 ...
 */
#ifndef ARK_COMMON_SAFEPOINT_PAGE_MANAGER_H
#define ARK_COMMON_SAFEPOINT_PAGE_MANAGER_H
...
public:
    ...
    void Init()
    {
        readablePage_ = reinterpret_cast<uint8_t*>(
            mmap(nullptr, panda::ARK_COMMON_PAGE_SIZE, PROT_READ, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0));
        unreadablePage_ = reinterpret_cast<uint8_t*>(
            mmap(nullptr, panda::ARK_COMMON_PAGE_SIZE, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0));
    ...
    ~SafepointPageManager()
    {
        LOGE_IF(UNLIKELY_CC(munmap(readablePage_, panda::ARK_COMMON_PAGE_SIZE) != EOK)) <<
            "munmap failed in SafepointPageManager readablePage destruction, errno: " << errno;
        LOGE_IF(UNLIKELY_CC(munmap(unreadablePage_, panda::ARK_COMMON_PAGE_SIZE) != EOK)) <<
            "munmap failed in SafepointPageManager unreadablePage destruction, errno: " << errno;
        LOGE_IF(UNLIKELY_CC(munmap(unreadablePageForRawData_, panda::ARK_COMMON_PAGE_SIZE) != EOK)) <<
...
#endif // ARK_COMMON_SAFEPOINT_PAGE_MANAGER_H
