...
{
    LOG_GC(FATAL) << "Send LongGCEvent is not support in macos/ios.";
}

void DFXHiSysEvent::SendRuntimeIncompatibleEvent([[maybe_unused]] JSThread *thread,
    [[maybe_unused]] IncompatibleType type)
{
    LOG_ECMA(WARN) << "Send RuntimeIncompatibleEvent is not support in macos/ios.";
}
...
{
    LOG_GC(WARN) << "Get CpuUsage is not support in mac.";
    return -1.0;
}
}  // namespace panda::ecmascript
