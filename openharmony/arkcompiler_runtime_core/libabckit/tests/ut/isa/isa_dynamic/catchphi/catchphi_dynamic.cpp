...
// Test: test-kind=api, api=IsaApiDynamicImpl::iCreateCatchPhi, abc-kind=ArkTS1, category=positive
TEST_F(LibAbcKitDynamicCatchPhiTest, CatchPhiDynamicValid)
{
    auto output = helpers::ExecuteDynamicAbc(ABCKIT_ABC_DIR "ut/isa/isa_dynamic/catchphi/catchphi_dynamic.abc",
                                             "catchphi_dynamic");
    EXPECT_TRUE(helpers::Match(output, "CATCH\n"));

    helpers::TransformMethod(
        ABCKIT_ABC_DIR "ut/isa/isa_dynamic/catchphi/catchphi_dynamic.abc",
        ABCKIT_ABC_DIR "ut/isa/isa_dynamic/catchphi/catchphi_dynamic_modified.abc", "main",
        [](AbckitFile * /*file*/, AbckitCoreFunction * /*method*/, AbckitGraph *graph) {
            auto *print = helpers::FindFirstInst(graph, ABCKIT_ISA_API_DYNAMIC_OPCODE_CALLARG1);
            auto *catchPhi = g_dynG->iCreateCatchPhi(graph, g_implG->iGetBasicBlock(print), 0);
            g_implG->iSetInput(print, catchPhi, 1);
        },
        [&]([[maybe_unused]] AbckitGraph *graph) {});

    output = helpers::ExecuteDynamicAbc(ABCKIT_ABC_DIR "ut/isa/isa_dynamic/catchphi/catchphi_dynamic_modified.abc",
                                        "catchphi_dynamic");
...
TEST_F(LibAbcKitDynamicCatchPhiTest, CatchPhiNoAccDynamicValid)
{
    ...
    helpers::TransformMethod(
        ABCKIT_ABC_DIR "ut/isa/isa_dynamic/catchphi/catchphi_dynamic.abc",
        ABCKIT_ABC_DIR "ut/isa/isa_dynamic/catchphi/catchphi_dynamic_noacc_modified.abc", "main",
        [](AbckitFile * /*file*/, AbckitCoreFunction * /*method*/, AbckitGraph *graph) {
            auto *print = helpers::FindFirstInst(graph, ABCKIT_ISA_API_DYNAMIC_OPCODE_CALLARG1);
            auto constantI64Impl = g_implG->gCreateConstantI32(graph, 42);
            auto *catchPhi = g_dynG->iCreateCatchPhi(graph, g_implG->iGetBasicBlock(print), 2, constantI64Impl, print);
            g_implG->iSetInput(print, catchPhi, 1);
        },
        [&]([[maybe_unused]] AbckitGraph *graph) {});

    output = helpers::ExecuteDynamicAbc(
        ABCKIT_ABC_DIR "ut/isa/isa_dynamic/catchphi/catchphi_dynamic_noacc_modified.abc", "catchphi_dynamic");
