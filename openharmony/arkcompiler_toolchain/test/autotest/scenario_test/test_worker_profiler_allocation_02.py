...
class TestWorkerProfilerAllocation02:
    """
    测试用例：多实例内存调优 Allocation 录制（-p dumpheap启动）
    测试步骤：
        1.  -p dumpheap方式拉起应用
    ...
    def setup_method(self):
        logging.info('Start running TestWorkerProfilerAllocation02: setup')

        self.log_path = rf'{os.path.dirname(__file__)}\..\log'
        self.hilog_file_name = 'test_worker_profiler_allocation_02.hilog.txt'
    ...
    def teardown_method(self):
        ...
        logging.info('TestWorkerProfilerAllocation02 done')

    def test(self, test_suite_worker_02_dumpheap):
        logging.info('Start running TestWorkerProfilerAllocation02: test')
        self.config = test_suite_worker_02_dumpheap
