...
import fs from "fs"
import { generic_package_archive_openlab, generic_package_upload_openlab } from "./openlab.mjs"
import { moveDirSync } from "./utils.mjs"
import { Command } from "commander"

const command = new Command("openlab")

command.command("upload")
    .description("Uploads package to openlab")
...
command.command("download")
    .description("Downloads package from openlab")
    .option("-p --package-name <name>", "package name")
...
async function download(options) {
    ...
    await generic_package_archive_openlab(options.packageName, options.packageVersion, fileName, options.destination, null, true)
    moveDirSync(path.join(options.destination, fileNameNoEx), options.destination)
}

async function upload(options) {
    await generic_package_upload_openlab(options.packageName, options.packageVersion, options.archive)
}
