...
const RESOURCES_MODULE = path.resolve(path.join(__dirname, "./generated/ets-test/__resources.generated.ts"))
...
const makeConfig = ({ os, arch }) => ({
    ...
    output: {
        filename: "js/arkoala-test-node.js",
        publicPath: "/",
    ...
    plugins: [
        new CopyPlugin({
            patterns: [
                // Copy resource files from resources/**/*, except HTML pages
                { from: "**/*", context: path.join(__dirname, ".") },
            ]
        }),
        new DefinePlugin({
        }),
        new ProvidePlugin({
            "$r": [RESOURCES_MODULE, "$r"],
            "$rawfile": [RESOURCES_MODULE, "$rawfile"]
        }),
    ],
